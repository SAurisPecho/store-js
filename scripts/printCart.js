...
export function printCart() {
      ...
      <div class = "productdetailsyprice">
        <div class ="product-details">
         ...
         <div class="product-price">
            ...
            <div class ="top">
                <input class ="product-input" type="number" name="quantity" min="1" id="${product.id}_${product.color}" value="${product.quantity}" >
                <button class="btn-delete" id="${product.id}" data-productid="${product.id}"><i class="fa-solid fa-trash-can" data-productid="${product.id}"></i></button>
            ...
            <div class="bottom">
                <div class ="subtotal" id="subtotal-${product.id}_${product.color}"></div>
            </div>
        </div>
      </div>
      `;
  ...
  assignButtons(); 
  initializeQuantityInputs(); // Inicializar inputs después de renderizar productos
}

// //Subtotal 
export function updateSubtotal(product) {
  const subtotalElement = document.getElementById(`subtotal-${product.id}_${product.color}`);
  if ( subtotalElement){
  subtotalElement.textContent = `Subtotal $ ${product.price * product.quantity}.00`;
  }
}
